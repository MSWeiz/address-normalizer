...
class DerivateDictionary:
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos3022 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos3022)
                pos = wrappos3022.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos3026 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos3026)
            pos = wrappos3026.value
            while cou > 0: 
                wrappos3024 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos3024)
                pos = wrappos3024.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos3023 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos3023)
                    pos = wrappos3023.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos3025 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos3025)
            pos = wrappos3025.value
        self.__m_buf = wr
