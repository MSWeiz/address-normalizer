...
class KeyBaseTable(BaseTable):
    ...
    def get_unique_key(self) -> int:
        ...
        while True:
            ...
            wrapdisp3045 = RefOutArgWrapper(0)
            wraplen3046 = RefOutArgWrapper(0)
            inoutres3047 = self.__read_index_info(self.__m_unique_key_position, wrapdisp3045, wraplen3046)
            disp = wrapdisp3045.value
            len0_ = wraplen3046.value
            if (not inoutres3047): 
                continue
            if (disp == (0) and len0_ == 0): 
                retVal3484 = self.__m_unique_key_position
                self.__m_unique_key_position += 1
                return retVal3484
        self.__m_unique_key_position = -1
        return max0_ + 1
    ...
    def read_key_data_len(self, key : int) -> int:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3048 = RefOutArgWrapper(0)
        wraplen3049 = RefOutArgWrapper(0)
        inoutres3050 = self.__read_index_info(key, wrapdisp3048, wraplen3049)
        disp = wrapdisp3048.value
        len0_ = wraplen3049.value
        if (not inoutres3050): 
            return -1
        else: 
            return len0_
    ...
    def read_key_data(self, key : int, max_len : int=0) -> bytearray:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3051 = RefOutArgWrapper(0)
        wraplen3052 = RefOutArgWrapper(0)
        inoutres3053 = self.__read_index_info(key, wrapdisp3051, wraplen3052)
        disp = wrapdisp3051.value
        len0_ = wraplen3052.value
        if (not inoutres3053): 
            if (log0_): 
    ...
    def remove_key_data(self, key : int) -> None:
        ...
        disp = 0
        len0_ = 0
        wrapdisp3054 = RefOutArgWrapper(0)
        wraplen3055 = RefOutArgWrapper(0)
        inoutres3056 = self.__read_index_info(key, wrapdisp3054, wraplen3055)
        disp = wrapdisp3054.value
        len0_ = wraplen3055.value
        if (not inoutres3056): 
            return
        if (disp == (0)): 
            return
    ...
    def update_part_of_data(self, key : int, data : bytearray, pos : int) -> None:
        disp = 0
        len0_ = 0
        wrapdisp3057 = RefOutArgWrapper(0)
        wraplen3058 = RefOutArgWrapper(0)
        inoutres3059 = self.__read_index_info(key, wrapdisp3057, wraplen3058)
        disp = wrapdisp3057.value
        len0_ = wraplen3058.value
        if (not inoutres3059): 
            return
    ...
    def update_start_of_data(self, key : int, data : bytearray) -> None:
        disp = 0
        len0_ = 0
        wrapdisp3060 = RefOutArgWrapper(0)
        wraplen3061 = RefOutArgWrapper(0)
        inoutres3062 = self.__read_index_info(key, wrapdisp3060, wraplen3061)
        disp = wrapdisp3060.value
        len0_ = wraplen3061.value
        if (not inoutres3062): 
            return
        self._m_data.position = disp
        self._m_data.write(data, 0, len(data))
    ...
    def __add_data(self, key : int, data : bytearray, dst : Stream) -> None:
        ...
        if (dst == self._m_data): 
            disp = 0
            len0_ = 0
            wrapdisp3063 = RefOutArgWrapper(0)
            wraplen3064 = RefOutArgWrapper(0)
            inoutres3065 = self.__read_index_info(key, wrapdisp3063, wraplen3064)
            disp = wrapdisp3063.value
            len0_ = wraplen3064.value
            if (inoutres3065): 
