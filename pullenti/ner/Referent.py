...
class Referent:
    ...
    def get_int_value(self, attr_name : str, def_value : int) -> int:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return def_value
        res = 0
        wrapres2991 = RefOutArgWrapper(0)
        inoutres2992 = Utils.tryParseInt(str0_, wrapres2991)
        res = wrapres2991.value
        if (not inoutres2992): 
            return def_value
        return res
    ...
    def add_occurence_of_ref_tok(self, rt : 'ReferentToken') -> None:
        self.add_occurence(TextAnnotation._new1106(rt.kit.sofa, rt.begin_char, rt.end_char, rt.referent))
    ...
    def add_occurence(self, anno : 'TextAnnotation') -> None:
        ...
        if (anno.occurence_of != self and anno.occurence_of is not None): 
            anno = TextAnnotation._new2994(anno.begin_char, anno.end_char, anno.sofa)
        if (self.__m_occurrence is None): 
            self.__m_occurrence = list()
    ...
    def clone(self) -> 'Referent':
        ...
        for r in self.slots: 
            rr = Slot._new2995(r.type_name, r.value, r.count)
            rr.owner = res
            res.slots.append(rr)
        return res
    ...
    def deserialize(self, stream : Stream, all0_ : typing.List['Referent'], sofa : 'SourceOfAnalysis', new_format : bool=False) -> None:
        ...
        i = 0
        while i < cou: 
            a = TextAnnotation._new2996(sofa, self)
