...
class PhoneHelper:
    ...
    def initialize() -> None:
        ...
        for line0 in Utils.splitString(str0_, '\n', False): 
            ...
            while i < len(cod): 
                dig = cod[i]
                nn = None
                wrapnn2759 = RefOutArgWrapper(None)
                inoutres2760 = Utils.tryGetValue(tn.children, dig, wrapnn2759)
                nn = wrapnn2759.value
                if (not inoutres2760): 
    ...
    def get_country_prefix(full_number : str) -> str:
        ...
        while i < len(full_number): 
            dig = full_number[i]
            nn = None
            wrapnn2761 = RefOutArgWrapper(None)
            inoutres2762 = Utils.tryGetValue(nod.children, dig, wrapnn2761)
            nn = wrapnn2761.value
            if (not inoutres2762): 
                break
