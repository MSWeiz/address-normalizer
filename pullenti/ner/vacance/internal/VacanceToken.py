...
class VacanceToken(MetaToken):
        ...
        while True:
            ...
            if (MiscHelper.can_be_start_of_sentence(t)): 
                ...
                if (t.chars.is_capital_upper): 
                    ...
                    ski = (None)
            if (ski is None): 
                ski = VacanceToken._new2885(t, t, (VacanceTokenType.SKILL if ty0 == VacanceTokenType.UNDEFINED else ty0))
                has_verb = False
                res.append(ski)
    ...
    def initialize() -> None:
        ...
        for s in ["ЖЕЛАТЕЛЬНО", "ПРИВЕТСТВОВАТЬСЯ", "ЯВЛЯТЬСЯ ПРЕИМУЩЕСТВОМ", "КАК ПЛЮС", "БУДЕТ ПРЕИМУЩЕСТВОМ", "БУДЕТ ЯВЛЯТЬСЯ ПРЕИМУЩЕСТВОМ", "МЫ ЦЕНИМ"]: 
            VacanceToken.M_TERMINS.add(Termin._new2895(s, VacanceTokenType.PLUS, True, True))
    ...
    @staticmethod
    def _new2885(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'VacanceTokenType') -> 'VacanceToken':
