...
class BusinessFactItem(MetaToken):
    ...
    def __try_parse(t : 'Token') -> 'BusinessFactItem':
        ...
        if (tok is not None): 
            ...
            if (ki != BusinessFactKind.UNDEFINED): 
                return BusinessFactItem._new651(t, tok.end_token, BusinessFactItemTyp.BASE, ki, tok.morph, tok.termin.tag2 is not None)
            ...
            while True:
                ...
                ki = (Utils.valToEnum(tok.termin.tag, BusinessFactKind))
                if (ki != BusinessFactKind.UNDEFINED): 
                    return BusinessFactItem._new652(t, tok.end_token, BusinessFactItemTyp.BASE, ki, tok.morph)
                tt = tok.end_token
        ...
        if (npt is not None): 
            if (((((npt.noun.is_value("АКЦИОНЕР", None) or npt.noun.is_value("ВЛАДЕЛЕЦ", None) or npt.noun.is_value("ВЛАДЕЛИЦА", None)) or npt.noun.is_value("СОВЛАДЕЛЕЦ", None) or npt.noun.is_value("СОВЛАДЕЛИЦА", None)) or npt.noun.is_value("АКЦІОНЕР", None) or npt.noun.is_value("ВЛАСНИК", None)) or npt.noun.is_value("ВЛАСНИЦЯ", None) or npt.noun.is_value("СПІВВЛАСНИК", None)) or npt.noun.is_value("СПІВВЛАСНИЦЯ", None)): 
                return BusinessFactItem._new652(t, npt.end_token, BusinessFactItemTyp.BASE, BusinessFactKind.HAVE, npt.morph)
        if (npt is not None): 
            if ((npt.noun.is_value("ОСНОВАТЕЛЬ", None) or npt.noun.is_value("ОСНОВАТЕЛЬНИЦА", None) or npt.noun.is_value("ЗАСНОВНИК", None)) or npt.noun.is_value("ЗАСНОВНИЦЯ", None)): 
                return BusinessFactItem._new652(t, npt.end_token, BusinessFactItemTyp.BASE, BusinessFactKind.CREATE, npt.morph)
        return None
    ...
    def initialize() -> None:
        ...
        for s in ["НАЛЕЖНИЙ", "КОНТРОЛЬОВАНИЙ", "ВЛАДЕЕМЫЙ", "ПЕРЕЙТИ ПІД КОНТРОЛЬ"]: 
            BusinessFactItem.__m_base_onto.add(Termin._new664(s, BusinessFactKind.HAVE, s, MorphLang.UA))
    ...
    @staticmethod
    def _new651(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BusinessFactItemTyp', _arg4 : 'BusinessFactKind', _arg5 : 'MorphCollection', _arg6 : bool) -> 'BusinessFactItem':
    ...
    @staticmethod
    def _new652(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'BusinessFactItemTyp', _arg4 : 'BusinessFactKind', _arg5 : 'MorphCollection') -> 'BusinessFactItem':
        res = BusinessFactItem(_arg1, _arg2)
        res.typ = _arg3
        res.base_kind = _arg4
        res.morph = _arg5
