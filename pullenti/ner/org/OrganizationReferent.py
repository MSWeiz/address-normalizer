...
class OrganizationReferent(Referent):
    ...
    def __correct_name(self, name : str, num : int) -> str:
        ...
        if (str.isdigit(name[0]) and name.find(' ') > 0): 
            i = 0
            wrapi2512 = RefOutArgWrapper(0)
            inoutres2513 = Utils.tryParseInt(name[0:0+name.find(' ')], wrapi2512)
            i = wrapi2512.value
            if (inoutres2513): 
                if (i > 1): 
                    num.value = i
            ...
            if (i >= 0 and name[i] == '.'): 
                ...
                inoutres2514 = Utils.tryParseInt(name[i + 1:], num)
                if (i > 0 and inoutres2514 and num.value > 0): 
                    if (i < 1): 
                        return None
    ...
    def add_name(self, name : str, remove_long_gov_names : bool=True, t : 'Token'=None) -> None:
        num = 0
        wrapnum2515 = RefOutArgWrapper(0)
        s = self.__correct_name(name, wrapnum2515)
        num = wrapnum2515.value
        if (s is None): 
    ...
    def add_name_str(self, name : str, typ : 'OrgItemTypeToken', cou : int=1) -> None:
        ...
        if (name is None): 
            ...
            if (typ.name is not None and Utils.compareStrings(typ.name, typ.typ, True) != 0 and ((len(typ.name) > len(typ.typ) or self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                num = 0
                wrapnum2516 = RefOutArgWrapper(0)
                s = self.__correct_name(typ.name, wrapnum2516)
                num = wrapnum2516.value
            ...
            if (typ is None or typ.is_not_typ): 
                ...
                if (typ.name is not None): 
                    num = 0
                    wrapnum2517 = RefOutArgWrapper(0)
                    ss = self.__correct_name(typ.name, wrapnum2517)
                    num = wrapnum2517.value
                    if (ss is not None): 
            ...
            if (((eq_number or eq_eponyms)) and ((self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None or org0_.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                ...
                for v in self._name_vars.items(): 
                    if (typ == ReferentsEqualType.DIFFERENTTEXTS and v[1]): 
                        continue
                    b = False
                    wrapb2519 = RefOutArgWrapper(False)
                    inoutres2520 = Utils.tryGetValue(org0_._name_vars, v[0], wrapb2519)
                    b = wrapb2519.value
                    if (not inoutres2520): 
                        continue
