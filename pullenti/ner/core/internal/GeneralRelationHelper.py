...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new733(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new733(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi736 = RefOutArgWrapper(None)
                inoutres737 = Utils.tryGetValue(all0_, a.name, wrapsi736)
                si = wrapsi736.value
                if (not inoutres737): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli734 = RefOutArgWrapper(None)
                    inoutres735 = Utils.tryGetValue(si, s, wrapli734)
                    li = wrapli734.value
                    if (not inoutres735): 
                        li = list()
                        si[s] = li
