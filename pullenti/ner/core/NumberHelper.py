...
class NumberHelper:
    ...
    def __try_parse(token : 'Token', prev_val : 'NumberToken'=None) -> 'NumberToken':
        ...
        if (val is not None): 
                ...
                if (str.isdigit(sss[0]) and len(sss) == 3): 
                    ...
                    while True:
                        ...
                        if (ttt.whitespaces_before_count == 1 and ttt.length_char == 3 and str.isdigit(ss[0])): 
                            ii = 0
                            wrapii849 = RefOutArgWrapper(0)
                            inoutres850 = Utils.tryParseInt(ss, wrapii849)
                            ii = wrapii849.value
                            if (not inoutres850): 
                                break
                            val2 += ss
            ...
            for k in range(4):
                if ((isinstance(et.next0_, TextToken)) and et.next0_.chars.is_letter): 
                    ...
                    break
            res = NumberToken._new851(token, et, val, typ, mc)
        ...
        while t is not None: 
            ...
            if (num is None): 
                break
            wrapj852 = RefOutArgWrapper(0)
            inoutres853 = Utils.tryParseInt(str(num.termin.tag), wrapj852)
            j = wrapj852.value
            if (not inoutres853): 
                break
    ...
    def try_parse_age(t : 'Token') -> 'NumberToken':
            ...
            if (nt_next is not None): 
                ...
                if (isinstance(t1, TextToken)): 
                    ...
                    if ((v == "ЛЕТ" or v == "ЛЕТИЯ" or v == "ЛЕТИЕ") or v == "РІЧЧЯ" or v == "ЖЫЛЖЫГЫ"): 
                        return NumberToken._new851(t, t1, nt.value, NumberSpellingType.AGE, t1.morph)
        ...
        if (LanguageHelper.ends_with_ex(s, "ЛЕТИЕ", "ЛЕТИЯ", "РІЧЧЯ", None)): 
            ...
            if (term is not None): 
                return NumberToken._new851(tt, tt, str(term.tag), NumberSpellingType.AGE, tt.morph)
        s = tt.lemma
        i = s.find("ЛЕТН")
        if (i < 0): 
            i = s.find("РІЧН")
        if (i > 3): 
            ...
            for i in range(len(ss) - 2, 2, -1):
                ...
                if (len(num0) > 1 and num0[len(num0) - 1] == '0' and len(num1) == 1): 
                    return NumberToken._new851(tt, tt, num0[0:0+len(num0) - 1] + num1, NumberSpellingType.AGE, tt.morph)
            else: i = 2
        return None
    ...
    def __analize_number_tail(tt : 'TextToken', val : str) -> 'MetaToken':
            ...
            if (((s == "<" or s == "(")) and (isinstance(tt.next0_, TextToken))): 
                ...
                if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
                    if (tt.next0_.next0_ is not None and tt.next0_.next0_.is_char_of(">)")): 
                        ...
                        return MetaToken._new859(tt, tt.next0_.next0_, mc)
            return None
        if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
            mc = MorphCollection()
            mc.class0_ = MorphClass.ADJECTIVE
            mc.language = MorphLang.EN
            return MetaToken._new859(tt, tt, mc)
        ...
        if (tt.morph.language.is_ua and mc is None and s == "Ї"): 
            mc = MorphCollection()
            mc.add_item(MorphBaseInfo._new862(MorphClass.ADJECTIVE))
        if (mc is not None): 
            return MetaToken._new859(tt, tt, mc)
        if ((((len(s) < 3) and not tt.is_whitespace_before and tt.previous is not None) and tt.previous.is_hiphen and not tt.previous.is_whitespace_before) and tt.whitespaces_after_count == 1 and s != "А"): 
            if (len(s) == 1 and s != "Й" and not LanguageHelper.is_cyrillic_vowel(s[0])): 
                ...
                return MetaToken._new859(tt, tt, MorphCollection._new864(MorphClass.ADJECTIVE))
        return None
    ...
    def try_parse_real_number(t : 'Token', attrs : 'NumberParseAttr'=NumberParseAttr.NO) -> 'NumberToken':
        ...
        if (t.next0_ is not None and t.next0_.is_value("ЦЕЛЫЙ", "ЦІЛИЙ")): 
            ...
            if ((isinstance(tt1, NumberToken)) or (((isinstance(tt1, TextToken)) and tt1.is_value("НОЛЬ", "НУЛЬ")))): 
                ...
                if (res0.real_value == 0): 
                    ...
                    dd = 0
                    wrapdd868 = RefOutArgWrapper(0)
                    inoutres869 = Utils.tryParseFloat(str0_, wrapdd868)
                    dd = wrapdd868.value
                    if (inoutres869): 
                        pass
                    else: 
                        wrapdd866 = RefOutArgWrapper(0)
                        inoutres867 = Utils.tryParseFloat(str0_.replace('.', ','), wrapdd866)
                        dd = wrapdd866.value
                        if (inoutres867): 
                            pass
                        else: 
                ...
                return res0
        d = 0
        wrapd871 = RefOutArgWrapper(0)
        tt = NumberHelper.__try_parse_float(Utils.asObjectOrNull(t, NumberToken), wrapd871, attrs)
        d = wrapd871.value
        if (tt is None): 
        ...
        if (tt.next0_ is not None and tt.next0_.is_value("ДЕСЯТОК", None)): 
            d *= (10)
            tt = tt.next0_
        return NumberExToken._new870(t0, tt, "", NumberSpellingType.DIGIT, NumberExType.UNDEFINED, d)
    ...
    def get_number_base_info(num_val : str, cla : 'MorphClass') -> 'MorphBaseInfo':
        ...
        Utils.tryParseInt(num_val, wrapn874)
        n = wrapn874.value
        res = MorphBaseInfo._new862(cla)
    ...
    def string_to_double(str0_ : str) -> float:
        ...
        if (str0_ == "NaN"): 
            return math.nan
        wrapres879 = RefOutArgWrapper(0)
        inoutres880 = Utils.tryParseFloat(str0_, wrapres879)
        res = wrapres879.value
        if (inoutres880): 
            return res
        wrapres877 = RefOutArgWrapper(0)
        inoutres878 = Utils.tryParseFloat(str0_.replace('.', ','), wrapres877)
        res = wrapres877.value
        if (inoutres878): 
            return res
        wrapres875 = RefOutArgWrapper(0)
        inoutres876 = Utils.tryParseFloat(str0_.replace(',', '.'), wrapres875)
        res = wrapres875.value
        if (inoutres876): 
            return res
        return None
