...
class TerminCollection:
    ...
    def __add_to_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                nod.children = dict()
            nn = None
            wrapnn936 = RefOutArgWrapper(None)
            inoutres937 = Utils.tryGetValue(nod.children, ch, wrapnn936)
            nn = wrapnn936.value
            if (not inoutres937): 
    ...
    def __remove_from_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                return
            nn = None
            wrapnn938 = RefOutArgWrapper(None)
            inoutres939 = Utils.tryGetValue(nod.children, ch, wrapnn938)
            nn = wrapnn938.value
            if (not inoutres939): 
                return
            nod = nn
    ...
    def __find_in_tree(self, key : str, lang : 'MorphLang') -> typing.List['Termin']:
        ...
        while i < len(key): 
            ...
            if (nod.children is not None): 
                wrapnn940 = RefOutArgWrapper(None)
                Utils.tryGetValue(nod.children, ch, wrapnn940)
                nn = wrapnn940.value
            if (nn is None): 
                if (ch == (32)): 
    ...
    def __add_to_hash1(self, key : int, t : 'Termin') -> None:
        li = None
        wrapli941 = RefOutArgWrapper(None)
        inoutres942 = Utils.tryGetValue(self.__m_hash1, key, wrapli941)
        li = wrapli941.value
        if (not inoutres942): 
            li = list()
    ...
    def __try_attach_all_(self, token : 'Token', pars : 'TerminParseAttr'=TerminParseAttr.NO, main_root : bool=False) -> typing.List['TerminToken']:
            ...
            if ((((pars) & (TerminParseAttr.TERMONLY))) != (TerminParseAttr.NO)): 
                ...
                while i < tt.invariant_prefix_length_of_morph_vars: 
                    ...
                    if (nod.children is None): 
                        ...
                        break
                    nn = None
                    wrapnn943 = RefOutArgWrapper(None)
                    inoutres944 = Utils.tryGetValue(nod.children, ch, wrapnn943)
                    nn = wrapnn943.value
                    if (not inoutres944): 
                        no_vars = True
                        break
                ...
                if ((no_vars and tt.term0 is not None and tt.term != tt.term0) and tt.invariant_prefix_length_of_morph_vars <= len(tt.term0)): 
                    ...
                    while i < tt.invariant_prefix_length_of_morph_vars: 
                        ...
                        if (nod.children is None): 
                            ...
                            break
                        nn = None
                        wrapnn945 = RefOutArgWrapper(None)
                        inoutres946 = Utils.tryGetValue(nod.children, ch, wrapnn945)
                        nn = wrapnn945.value
                        if (not inoutres946): 
                            no_vars = True
                            break
                        ...
                        i += 1
            if (not no_vars): 
                wrapres951 = RefOutArgWrapper(res)
                inoutres952 = self.__manage_var(token, pars, s, nod, len0, wrapres951)
                res = wrapres951.value
                if (inoutres952): 
                    was_vars = True
                i = 0
                first_pass3240 = True
                while True:
                    ...
                    if (wf.normal_case is None or wf.normal_case == s): 
                        ...
                        if (j < i): 
                            ok = False
                    if (ok): 
                        wrapres947 = RefOutArgWrapper(res)
                        inoutres948 = self.__manage_var(token, pars, wf.normal_case, nod, tt.invariant_prefix_length_of_morph_vars, wrapres947)
                        res = wrapres947.value
                        if (inoutres948): 
                            was_vars = True
                    ...
                    if (j < i): 
                        continue
                    wrapres949 = RefOutArgWrapper(res)
                    inoutres950 = self.__manage_var(token, pars, wf.normal_full, nod, tt.invariant_prefix_length_of_morph_vars, wrapres949)
                    res = wrapres949.value
                    if (inoutres950): 
                        was_vars = True
        elif (isinstance(token, NumberToken)): 
            wrapres953 = RefOutArgWrapper(res)
            inoutres954 = self.__manage_var(token, pars, str(token.value), root, 0, wrapres953)
            res = wrapres953.value
            if (inoutres954): 
                was_vars = True
        else: 
            return None
        if (not was_vars and s is not None and len(s) == 1): 
            vars0_ = [ ]
            wrapvars955 = RefOutArgWrapper(None)
            inoutres956 = Utils.tryGetValue(self.__m_hash1, ord(s[0]), wrapvars955)
            vars0_ = wrapvars955.value
            if (inoutres956): 
                for t in vars0_: 
                    if (not t.lang.is_undefined): 
                        if (not token.morph.language.is_undefined): 
    ...
    def __manage_var(self, token : 'Token', pars : 'TerminParseAttr', v : str, nod : 'CharNode', i0 : int, res : typing.List['TerminToken']) -> bool:
        ...
        while i < len(v): 
            ...
            if (nod.children is None): 
                return False
            nn = None
            wrapnn957 = RefOutArgWrapper(None)
            inoutres958 = Utils.tryGetValue(nod.children, ch, wrapnn957)
            nn = wrapnn957.value
            if (not inoutres958): 
                return False
            nod = nn
    ...
    def find_termins_by_canonic_text(self, text : str) -> typing.List['Termin']:
        if (self.__m_hash_canonic is None): 
            ...
            for t in self.termins: 
                ...
                inoutres960 = Utils.tryGetValue(self.__m_hash_canonic, ct, wrapli959)
                li = wrapli959.value
                if (not inoutres960): 
                    li = list()
                ...
                if (not t in li): 
                    li.append(t)
        res = [ ]
        wrapres961 = RefOutArgWrapper(None)
        inoutres962 = Utils.tryGetValue(self.__m_hash_canonic, text, wrapres961)
        res = wrapres961.value
        if (not inoutres962): 
            return None
        else: 
            return res