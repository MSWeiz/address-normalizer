...
class ChatItemToken(MetaToken):
    ...
    def try_parse(t : 'Token') -> 'ChatItemToken':
        ...
        while True:
            ...
            if (isinstance(tt.get_referent(), DateReferent)): 
                ...
                wrapdt0701 = RefOutArgWrapper(None)
                wrapdt1702 = RefOutArgWrapper(None)
                ok = dr.calculate_date_range((datetime.datetime.now() if ProcessorService.DEBUG_CURRENT_DATE_TIME is None else ProcessorService.DEBUG_CURRENT_DATE_TIME), wrapdt0701, wrapdt1702, 0)
                dt0 = wrapdt0701.value
                dt1 = wrapdt1702.value
                ...
                ok = dr.calculate_date_range((datetime.datetime.now() if ProcessorService.DEBUG_CURRENT_DATE_TIME is None else ProcessorService.DEBUG_CURRENT_DATE_TIME), wrapdt0703, wrapdt1704, 0)
                dt0 = wrapdt0703.value
                dt1 = wrapdt1704.value
            if (ok): 
                if (dt0 != dt1): 
                    res = ChatItemToken._new705(tt, tt, ChatType.DATERANGE)
                    ...
                    res = ChatItemToken._new705(tt, tt, ChatType.DATE)
                    res.value = "{0}.{1}.{2}".format(dt0.year, "{:02d}".format(dt0.month), "{:02d}".format(dt0.day))
                    if (dt0.hour > 0 or dt0.minute > 0): 
                        res.value = "{0} {1}:{2}".format(res.value, "{:02d}".format(dt0.hour), "{:02d}".format(dt0.minute))
                    return res
    ...
    @staticmethod
    def _new705(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ChatType') -> 'ChatItemToken':
