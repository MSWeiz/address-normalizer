...
class OrgItemToken(ReferentToken):
        ...
        while t is not None: 
            ...
            if (org0_ is not None): 
                if (d is None): 
                    d = GeoTokenData(t)
                d.org0_ = org0_
                if (org0_.has_terr_keyword or org0_.not_geo or ((org0_.is_gsk and not org0_.keyword_after and not org0_.not_org))): 
    ...
    def __try_parse(t : 'Token', after_terr : bool, lev : int, ad : 'GeoAnalyzerData') -> 'OrgItemToken':
        ...
        if (tok_typ is not None and tok_typ.begin_token.is_value("СП", None)): 
            ...
            if (cits is not None and len(cits) == 2 and cits[0].typ == CityItemToken.ItemType.NOUN): 
                return None
        if (tok_typ is not None and tok_typ.begin_token.is_value("МАГАЗИН", None)): 
            return None
    ...
    def __try_parse_railway(t : 'Token') -> 'StreetItemToken':
        ...
        if (rt0 is not None): 
            res = StreetItemToken._new1308(t, rt0.end_token, StreetItemType.FIX, True)
    ...
    def __try_parse_rzd_dir(t : 'Token') -> 'StreetItemToken':
        ...
        if (tt0 is None): 
            return None
        res = StreetItemToken._new1309(tt0, tt1, StreetItemType.FIX, True, 1)
        if (val is not None): 
            res.value = val
        else: 
    ...
    def __try_parse_details(self) -> None:
        ...
        if (tok is not None): 
            ...
            if (ait is not None and ait.typ == AddressItemType.STREET and ait.referent is not None): 
                ...
                ait = AddressItemToken.try_parse(t, False, None, None)
                if (ait is not None and ait.typ == AddressItemType.HOUSE and ait.value is not None): 
