...
class NameToken(MetaToken):
    ...
    def try_parse(t : 'Token', ty : 'GeoTokenType', lev : int, after_typ : bool=False) -> 'NameToken':
        ...
        if (br is not None): 
            ...
            res = NameToken._new1281(t, t, True)
            ...
            if (isinstance((ttt), NumberToken)): 
                res = NameToken._new1282(t, ttt, ttt.value)
                ...
                if ((num) is not None): 
                    res = NameToken._new1283(t, num.end_token, num.value + " ЛЕТ")
                else: 
                    ...
                    if ((num) is not None): 
                        ...
                        if (nn is not None and not MiscLocationHelper.is_user_param_address(t)): 
                            if (nn.ex_typ != NumberExType.UNDEFINED): 
                                return None
                        res = NameToken._new1282(t, t, t.value)
                        ...
                        if (year is not None): 
                            ...
                            if (mon is not None): 
                                res = NameToken._new1289(t, t, str(t.get_referent()).upper())
                        ...
                        if ((t.get_morph_class_in_dictionary().is_preposition and t.chars.is_all_upper and t.whitespaces_after_count > 0) and (t.whitespaces_after_count < 3) and (isinstance(t.next0_, TextToken))): 
                            ...
                            if (npt is not None and npt.end_token != t): 
                                return NameToken._new1290(t, npt.end_token, True, MiscHelper.get_text_value(t, npt.end_token, GetTextAttr.NO))
                        if ((t.chars.is_all_upper and t.next0_ is not None and t.next0_.is_hiphen) and (isinstance(t.next0_.next0_, TextToken))): 
                            return NameToken._new1290(t, t.next0_.next0_, True, MiscHelper.get_text_value(t, t.next0_.next0_, GetTextAttr.NO))
                        ok = False
                        ...
                        if (ty == GeoTokenType.ORG and ok and t.chars.is_letter): 
                            return NameToken._new1289(t, t, t.term)
                        ...
                        if (next0__ is not None and next0__.number is not None and next0__.name is None): 
                            ...
                            while tt is not None: 
                                ...
                                tt = tt.next0_
                            if (nam.tell() >= 3): 
                                res = NameToken._new1289(t, t1, Utils.toStringStringIO(nam))
                            else: 
                                rt = t.kit.process_referent("PERSON", t, None)
                                if (rt is not None): 
                                    res = NameToken._new1289(t, rt.end_token, rt.referent.get_string_value("LASTNAME"))
                                    if (res.name is None): 
                        ...
                        if ((ttok) is not None): 
                            res = NameToken._new1289(t, ttok.end_token, ttok.termin.canonic_text)
                                ...
                                if (npt is not None and npt.end_token.is_value("КВАРТАЛ", None)): 
                                    if (npt.begin_token == npt.end_token or npt.begin_token.is_value("КАДАСТРОВЫЙ", None)): 
                                        ...
                                        if (num2 is not None and num2.is_cadaster_number): 
                                            res = NameToken._new1282(t, num2.end_token, num2.value)
                                ...
                                if (npt is not None and npt.end_token.chars.is_all_lower): 
                                    if (t.chars.is_all_lower): 
                                        ...
                                        if (npt.morph.number == MorphNumber.PLURAL): 
                                            ...
                                            npt = (None)
                                if (npt is not None): 
                                    res = NameToken._new1297(t, npt.end_token, npt.morph, MiscHelper.get_text_value_of_meta_token(npt, GetTextAttr.NO).replace("-", " "))
                                    ...
                                    if ((ait is not None and ait.typ != AddressItemType.NUMBER and ait.value is not None) and ait.value != "0"): 
                                        return None
                                    res = NameToken._new1298(t, t, t.term, t.morph)
                                    if (t.chars.is_all_lower): 
                                        res.is_doubt = True
    ...
    def check_initial_and_surname(ini : str, sur : 'Token') -> bool:
        ...
        if (ini == "А"): 
            if ((((((((sur.is_value("МАТРОСОВ", None) or sur.is_value("ПУШКИН", None) or sur.is_value("УЛЬЯНОВ", None)) or sur.is_value("СУВОРОВ", None) or sur.is_value("АХМАТОВА", None)) or sur.is_value("КАДЫРОВ", None) or sur.is_value("АБУБАКАРОВ", None)) or sur.is_value("АЛИША", None) or sur.is_value("БЛОК", None)) or sur.is_value("ГАЙДАР", None) or sur.is_value("НЕВСКИЙ", None)) or sur.is_value("НЕВСКИЙ", None) or sur.is_value("СУЛТАН", None)) or sur.is_value("ТОЛСТОЙ", None) or sur.is_value("ШЕРИПОВ", None)) or sur.is_value("ГРИН", None)): 
                return True
        if (ini == "Б"): 
            if (sur.is_value("ХМЕЛЬНИЦКИЙ", None)): 
                return True
        if (ini == "В" or ini == "B"): 
            if (((((sur.is_value("ЛЕНИН", None) or sur.is_value("ТЕРЕШКОВА", None) or sur.is_value("УЛЬЯНОВ", None)) or sur.is_value("ВЫСОЦКИЙ", None) or sur.is_value("ПАСТЕРНАК", None)) or sur.is_value("ЧАПАЕВ", None) or sur.is_value("ЧКАЛОВ", None)) or sur.is_value("ЭМИРОВ", None) or sur.is_value("ШУКШИН", None)) or sur.is_value("МАЯКОВСКИЙ", None)): 
                return True
        if (ini == "Г"): 
    ...
    def initialize() -> None:
        ...
        t = Termin._new1299("МИНИСТЕРСТВО ОБОРОНЫ", "МО")
        ...
        for s in NameToken._standard_names: 
            ...
            NameToken.M_ONTO.add(t)
        t = Termin("ПАРТСЪЕЗДА")
        t.add_abridge("П/СЪЕЗДА")
        t.add_variant("ПАРТИЙНОГО СЪЕЗДА", False)
        t.add_abridge("ПАРТ.СЪЕЗДА")
        NameToken.M_ONTO.add(t)
    
    @staticmethod
    def _new1281(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.is_doubt = _arg3
        return res
    
    @staticmethod
    def _new1282(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.number = _arg3
        return res
    
    @staticmethod
    def _new1283(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.pref = _arg3
        return res
    
    @staticmethod
    def _new1287(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : bool) -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.number = _arg3
        res.is_doubt = _arg4
        return res
    ...
    @staticmethod
    def _new1289(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str) -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.name = _arg3
        return res
    
    @staticmethod
    def _new1290(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : str) -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.is_doubt = _arg3
        res.name = _arg4
        return res
    ...
    @staticmethod
    def _new1297(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : str) -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.morph = _arg3
        res.name = _arg4
        return res
    ...
    @staticmethod
    def _new1298(_arg1 : 'Token', _arg2 : 'Token', _arg3 : str, _arg4 : 'MorphCollection') -> 'NameToken':
        res = NameToken(_arg1, _arg2)
        res.name = _arg3
        res.morph = _arg4
        return res
