...
class GeoReferent(Referent):
    ...
    def __is_city(v : str) -> bool:
        if (((((((((((((("поселок" in v or "селение" in v or "городок" in v) or "село" in v or "деревня" in v) or "станица" in v or "пункт" in v) or "станция" in v or "аул" in v) or "улус" in v or "наслег" in v) or "хутор" in v or "слобода" in v) or "местечко" in v or "усадьба" in v) or "починок" in v or "заимка" in v) or "аал" in v or "выселки" in v) or "арбан" in v or v == "участок") or "місто" in v or "селище" in v) or "сіло" in v or "станиця" in v) or "станція" in v or "city" in v) or "municipality" in v or "town" in v): 
            return True
        if ("порт" in v): 
            return True
